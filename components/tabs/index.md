...
| variant | 页签形态 | `"sample" \| "line" \| "card" \| "browser"` | `line` |
...
| onRemove | tab关闭时的回调，返回关闭的tab的key值 | `(name: string) => void` | - |
| onChange | 切换面板的回调 | `(name: string) => void` | - |
| onTabClick | tab点击时的回调 | `(name: string) => void` | - |
