# ColorPicker 颜色选择器

可以自由的输出颜色。

## 何时使用

- 需要自定义颜色时

## 代码演示

[基本用法](./demo/basic.vue)

- 点击打开颜色面板

[尺寸大小 / 不可用](./demo/size.vue)

- `small` 为小尺寸， `large` 为大尺寸

[主题与形状](./demo/appearance.vue)

- 支持 `outline`、`fill`、`plain` 主题，以及 `round`、`circle`、`square` 形状

[自定义触发器](./demo/custom-trigger.vue)

- 自定义颜色面板的触发器。

[弹出位置](./demo/placement.vue)

- 支持 6 个弹出位置 , 如果上面的空间不够，色盘会自动在下面展示

## API

| 属性 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| modelValue | 颜色值,可以使用 `v-model` 双向绑定 | `string` | - |
| value | 初始值，仅初始化时读取；后续更新使用 modelValue，同时传入时 modelValue 优先。 | `string` | - |
| opened | 默认是否展开颜色面板 | `boolean` | false |
| mode | 颜色展示类型,提供 3 种模式(`hex` 、 `rgb` 、`hsl`) | `"hex" \| "rgb" \| "hsl"` | 'hex' |
| presets | 自定义颜色盘 | `string[]` | - |
| disabledAlpha | 是否禁用透明 | `boolean` | false |
| disabled | 是否失效状态 | `boolean` | false |
| readonly | 是否只读，不可展开或修改 | `boolean` | false |
| trigger | 下拉触发形式 | `"hover" \| "click"` | hover |
| showText | 是否展示颜色文字 | `boolean` | false |
| size | 颜色选择器的尺寸 | `"small" \| "medium" \| "large"` | - |
| theme | 外观主题，支持 `outline`、`fill`、`plain`，未设置时可继承 Form | `"default" \| "fill" \| "outline" \| "plain" \| "solid" \| "dashed" \| "underlined"` | outline |
| shape | 形状，支持 `round`、`circle`、`square`，未设置时可继承 Form | `"default" \| "circle" \| "square" \| "round"` | - |
| placement | 颜色选择器的放置位置 | `"top" \| "top-left" \| "top-right" \| "bottom" \| "bottom-left" \| "bottom-right"` | bottom-left |
| onUpdateMode | 颜色模式更新时触发 | (mode: ColorMode) => void | - |
| onChange | 颜色值改变的时候触发,返回颜色的值 | `(value: string) => void` | - |
| onOpenChange | 颜色选择器展开或者收起的时候触发 | `(open: boolean) => void` | - |
| panelOnly | 仅渲染颜色选择面板，不显示触发输入框 | `boolean` | false |
