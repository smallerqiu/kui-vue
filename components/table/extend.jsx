...
export default {
  ...
  mounted() {
    let { thead } = this.$refs;
    if (thead && typeof window !== "undefined") {
  ...
  methods: {
    ...
    renderTH(col, left, right) {
      ...
      let inner = hasCheckbox && col.checkType !== "radio" ? <Checkbox indeterminate={this.indeterminate} checked={this.checkAll} onChange={this.onSelectAll} /> : col.$title || col.title;
      return (
        <th {...props}>
      ...
      let head = [],
        ...
        right =
          cols
            ...
            .map((c) => c.width)
            .reduce((a, b) => a + b, 0) + (this.height ? this.scrollBarHeight : 0);
