...
| 属性 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| modelValue | 指定选中项目的 `value` 值，可以使用 `v-model` 双向绑定数据 | `TreeSelectValue` | - |
| value | 初始值，仅初始化时读取；后续更新使用 modelValue，同时传入时 modelValue 优先。 | `TreeSelectValue` | - |
| width | 组件宽度 | `number` | - |
...
| virtualHeight | 虚拟滚动下拉视口高度 | `string \| number` | 260 |
