# Icon 图标

5.x 版本重新引入了图标集, 支持更多的图标。
使用图标组件，需要安装 `kui-icons` 图标组件包：

```bash
npm install --save kui-icons
```

使用

```html
<template>
  <Icon :type="Heart" />
</template>
<script setup lang="ts">
  import { Heart } from "kui-icons";
</script>
```

[IconList](./demo/search.tsx?demo=false)

- 搜索并浏览 `kui-icons` 提供的全部图标。

[基本用法](./demo/basic.vue)

- 可以通过 `type`、`size` 、`color` 属性分别设置图标的类型、大小、颜色,也可以通过设置 `spin` 属性来实现动画旋转效果。

## API

| 属性 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| type | 图标类型。遵循图标的命名规范 | `IconType[]` | - |
| size | 图标的大小，单位是 px | `string \| number` | - |
| color | 图标的颜色 | `string` | - |
| spin | 是否有旋转动画 | `boolean` | false |
| strokeWidth | 图标的线条粗细 | `string \| number` | 2 |
| onClick | 点击事件 | `(event: MouseEvent) => void` | - |
| reverseFill | 图标边框和反相填充,仅支持闭合的图标 | `boolean` | false |
| role | 无障碍角色 | `string` | - |
| tabindex | 键盘聚焦顺序 | `number` | - |
| aria-label | 无障碍标签 | `string` | - |
| onPointerdown | 指针按下时触发 | `((event: PointerEvent) => void)` | - |
| onKeydown | 键盘按下时触发 | `((event: KeyboardEvent) => void)` | - |
| onPointerup | 指针抬起时触发 | `((event: PointerEvent) => void)` | - |
