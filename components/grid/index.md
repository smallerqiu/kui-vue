...
[仪表盘卡片布局 (Auto-fill + Min-Width)](./demo/autoFillMinWidth.vue)
...
[复杂表单响应式 (Breakpoint Fallback)](./demo/breakpointFallback.vue)
- 断点的向上/向下寻找逻辑。例如：定义了 md 没定义 lg，系统是否能正确应用 md 的值。

[圣杯布局/管理后台 (Fixed Rows & Areas)](./demo/fixedRowsAreas.vue)
- `rows` 和 `rowSpan` 的垂直控制力。

[响应式底栏 / 菜单（Footer Strategy）](./demo/footerStrategy.vue)
- 从“多列垂直排列”到“单列折叠”的极致压缩。

[Hero Section 叠加布局（Layering）](./demo/heroSection.vue)
- `grid-column-start` 和 `grid-row-start` 的重叠能力,文字浮动在图片某处

[响应式隐藏与强制排序 (Suffix & Display None)](./demo/suffixDisplayNone.vue)
