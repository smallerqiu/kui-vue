...
[仪表盘卡片布局 (Auto-fill + Min-Width)](./demo/auto-fill-min-width.vue)

- 无需手动设置断点，依靠 `itemMinWidth` 让容器根据宽度自动增减列数。 > 当设置了 `itemMinWidth` 时，cols` 参数将失效。这是一种内容驱动的布局方式，非常适合图片画廊或卡片列表，它能保证在容器缩放时，卡片始终保持合适的宽度而不会过于拥挤。 >

[复杂表单响应式 (Breakpoint Fallback)](./demo/breakpoint-fallback.vue)

- 断点的向上/向下寻找逻辑。例如：定义了 md 没定义 lg，系统是否能正确应用 md 的值。

[圣杯布局/管理后台 (Fixed Rows & Areas)](./demo/fixed-rows-areas.vue)

- `rows` 和 `rowSpan` 的垂直控制力。

[响应式隐藏与强制排序 (Suffix & Display None)](./demo/suffix-display-none.vue)

- `span: 0` 彻底移除 DOM 占位，`suffix` 跨越所有动态项。

[图片画廊 / 瀑布流（Bento Grid 风格）](./demo/bento.vue)

- 非对称布局

[复杂数据详情页（对齐与留白）](./demo/align-white-space.vue)

- 描述列表（Descriptions）的效果，但在不同屏幕下自动调整展示密度。

[Hero Section 叠加布局（Layering）](./demo/hero-section.vue)

- `grid-column-start` 和 `grid-row-start` 的重叠能力,文字浮动在图片某处

[响应式底栏 / 菜单（Footer Strategy）](./demo/footer-strategy.vue)

- 从“多列垂直排列”到“单列折叠”的极致压缩。

[布局层级设计 (Architecture)](./demo/architecture.vue)

- - 第一层：Layout 系列 —— 页面大骨架（Header + Sider + Content）。 - 第二层：Grid 系列 —— 内容区的主要 2D 布局（统计卡片、Bento 图表矩阵）。 - 第三层：Row/Col —— 简单的 1D 比例划分（如底部的两个并排表格）。 - 第四层：Flex —— 极细微的内容对齐（卡片标题 + 图标）。
