...
export function createRipple(
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        // in the same position when the canvas moves or the page scrolls.
        if (transform) content.style.transform = transform.toString();
        contentDirty = true;
        wake();
      } catch (error) {
        console.error("Ripple failed to capture its HTML content:", error);
      }
    };

    // HTML-in-Canvas keeps a pixel snapshot of the subtree. Changes to theme
    // attributes on an ancestor do not reliably emit a paint event in current
    // experimental implementations, so explicitly refresh after CSS tokens
    ...
    for (let element = source.parentElement; element; element = element.parentElement) {
      appearanceObserver.observe(element, {
        attributes: true,
        attributeFilter: ["theme-mode", "shape-mode", "class", "style"],
      });
    }
  }
  ...
  return {
    setOptions(next) {
      if (
        !Object.entries(next).some(([key, value]) => config[key as keyof RippleOptions] !== value)
      )
        return;
