...
先在应用项目中安装 `kui-vue` 及其依赖（见上面的安装命令），并确认安装版本包含
`node_modules/kui-vue/ai/mcp.mjs`。不要只复制这个脚本，它还需要同包的元数据和依赖。
...
才使用 `"command": "pnpm"` 和 `"args": ["exec", "kui-vue-mcp"]`。
...
`kui-vue/metadata` 和 `kui-vue/skill` 是 npm 包的导出路径，**不是目录**。
可以运行 `node -p "require.resolve('kui-vue/metadata')"` 定位文件，也可以使用
