<template>
  <div :class="`k-${type}`">
    <k-notice v-for="(item,i) in notices" :key="i" :type="item.type" :title="item.title" :transition-name="item.transitionName" :content="item.content" :closable="item.closable" :close="item.close" :noticeType="item.noticeType">
