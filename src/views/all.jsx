        ...
        let card = (
          <Col span={6}>
            <router-link to={`/${item.key}/${item.name}`}>
              <Card bordered title={item.sub + " " + item.title}>
  ...
  render() {
    ...
    return (
      <div class="all-components">
        <h1>组件</h1>
        <p>
          <code>kui</code> 提供了65款组件，之后会根据需求补充，欢迎提供建议！
        </p>
        {nav.map((item, x) => {
          return [<h2>{item.title}</h2>, this.renders(item.children)];
        })}
      </div>
    );
  },
