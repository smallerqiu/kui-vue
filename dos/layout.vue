<template>
  <section class="body">
    <header>
      ...
      <Menu style="float:right;" mode="horizontal" activeName="/install" @onSelect="go">
        <MenuItem name="/" icon="">首页</MenuItem>
        <MenuItem name="/install" icon="">组件</MenuItem>
        <MenuItem name="https://www.chuchur.com" icon="">Blog</MenuItem>
      </Menu>
    </header>
    <section class="main">
      <Row>
        <Col span="4">
        <nav class="nav">
          <Menu @onSelect="go" :activeName="activeName" width="auto">
            <MenuGroup :title="item.title" v-for="(item,x) in nav" :name="item.title" :key="x">
              <MenuItem v-for="(sub,y) in item.child" :icon="sub.icon" :name="sub.link||sub.weblink" :key="y">{{sub.title}}<span class="sub">{{sub.sub}}</span></MenuItem>
           </MenuGroup>
          </Menu>
          <!-- <menu>
